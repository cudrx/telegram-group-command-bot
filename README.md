...
Бот работает от явных команд, хранит журнал сообщений, умеет обращаться к OpenAI-compatible LLM, при необходимости использует поиск через Tavily, кэширует результаты распознавания медиа и озвучивает ответы через Yandex SpeechKit.
...
- Команды: `/summarize`, `/decide`, `/answer`, `/translate`, `/read`, `/meme`.
