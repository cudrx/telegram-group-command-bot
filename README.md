...
Остальные настройки времени выполнения имеют значения по умолчанию в `src/config/env/` и могут переопределяться через окружение, если они есть в схеме.
...
- `src/app/chat-orchestrator/` — обработка команд, задачи ответа, поиск, медиа-контекст, голосовые ответы.
