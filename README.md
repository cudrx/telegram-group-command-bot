...
- Команды: `/summarize`, `/decide`, `/answer`, `/read`, `/meme`, `/weekly` только для администратора.
...
- `/meme` берет случайный свежий image/gif-мем через `meme-api.com` по hardcoded пулу сабреддитов, локализует подпись через LLM и отправляет медиа в чат.
...
- `src/app/chat-orchestrator/` — обработка команд, задачи ответа, поиск, медиа-контекст, голосовые ответы и `/meme`.
