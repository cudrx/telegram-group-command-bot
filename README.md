...
Бот работает от явных команд, хранит журнал сообщений, умеет обращаться к OpenAI-compatible LLM, при необходимости использует поиск через Tavily, кэширует результаты распознавания медиа, делает недельный обзор и озвучивает ответы через Yandex SpeechKit.
...
- SQLite хранит чаты, сообщения, данные отправителей, reply-связи, отметки редактирования, артефакты медиа и небольшой `app_state`.
- Команды: `/summarize`, `/decide`, `/answer`, `/translate`, `/read`, `/meme`, `/weekly` только для администратора.
...
- GIF/animation-мемы распознаются через JPEG-кадр примерно на первой секунде
  при наличии локального `ffmpeg`.
...
- `/meme` берет случайный свежий image/gif-мем через `meme-api.com` по hardcoded пулу сабреддитов, отправляет медиа с оригинальным title и сохраняет распознавание отправленного медиа для будущего контекста.
- Локальные подсказки и fallback-сообщения бота отправляются только текстом, даже если исходящая озвучка включена.
- `/weekly` читает последние семь дней из SQLite, использует только сохраненные артефакты медиа и публикует обзор в рабочую группу.
- Безопасное HTML-форматирование ответов для Telegram.
- Оповещение о продакшн-деплое, дедуплицированное через SQLite.

Обычное упоминание бота и обычный текст в личке не запускают LLM. В проекте нет самостоятельных вмешательств, памяти о пользователях, профилей, алиасов, настроек по чатам и фоновых LLM-задач.
...
- `/answer` — ответить на сообщение, на которое пользователь сделал reply командой.
...
- `/meme` — отправить случайный неповторявшийся за последние 14 дней image/gif-мем из Reddit wrapper.
- `/weekly` — только личный чат администратора; публикует недельный обзор в `TELEGRAM_CHAT_ID`.
...
- `ffmpeg` для извлечения кадра из GIF/MP4-мемов перед распознаванием
...
Отчеты intent eval пишутся в `.eval-runs/`; папка игнорируется Git.

## Структура

- `src/index.ts` — точка входа процесса.
- `src/app.ts` — сборка приложения.
- `src/app/` — оркестрация, отправка сообщений в Telegram, недельный обзор, HTML-форматирование, оповещения о деплое.
- `src/app/chat-orchestrator/` — обработка команд, задачи ответа и перевода, поиск, медиа-контекст, голосовые ответы и `/meme`.
- `src/app/weekly/` — данные для недельного обзора, поиск событий, отбор и форматирование.
...
- `scripts/` — миграции, metadata для деплоя, eval-скрипты, smoke-проверка weekly.
