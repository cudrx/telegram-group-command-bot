...
Ближайший post-v1 фокус уже определён, но ещё не реализован в runtime:

- managed internet lookup для `/explain` и factual `/decide`;
- распознавание изображений;
- transcription/normalization для audio, voice messages и Telegram video notes;
- передача распознанного media-текста в тот же explicit-command pipeline без включения autonomous replies.
...
- понять, нужен ли minimal pending queue для нескольких одновременных explicit triggers, особенно когда lookup/media calls станут медленнее обычного reply path;
- спроектировать internet-backed `/explain` и factual `/decide` как следующий этап после стабилизации v1;
