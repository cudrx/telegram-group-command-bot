...
- `/meme` не сохраняет скачанные Reddit-медиа на диск постоянно: файлы живут только во временной папке до отправки в Telegram, а anti-repeat хранит только metadata поста.
...
10. Для `/meme` запускается отдельный Reddit-поток выбора, скачивания, локализации подписи и отправки media.
11. Для `/summarize`, `/decide`, `/answer` собирается контекст, добавляется текущая дата и время Москвы, при необходимости добавляется контекст поиска и медиа, затем вызывается LLM.
...
- Источник — недельный Reddit top из hardcoded пула сабреддитов.
- На один запуск выбираются до трех разных сабреддитов; для каждого запрашивается `top.json?t=week&limit=10`.
- Уже отправленные за последние 14 дней post ids отбрасываются по `meme_posts`.
- Поддерживаются Reddit-hosted images, galleries, GIF/animation и Reddit video `fallback_url`; external links пропускаются.
- Gallery сохраняет порядок Reddit `gallery_data`; если после фильтра остается один элемент, он отправляется как одиночное фото, иначе как Telegram media group до 10 фото с подписью на первом элементе.
- Видео и картинки скачиваются во временные файлы, отправляются в Telegram и чистятся в `finally`.
- Reddit-video assembler сейчас использует pass-through `fallback_url`; место под сборку audio/video вынесено отдельно.
