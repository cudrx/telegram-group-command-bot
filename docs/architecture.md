...
- TTS не решает содержание ответа: текст сначала генерируется или берется из сообщения, на которое сделали reply, затем локальная политика решает, можно ли отправить voice.
...
Статический текст prompt живет в `llm/`.
...
10. Для `/summarize`, `/decide`, `/answer` собирается контекст, при необходимости добавляется контекст поиска и медиа, затем вызывается LLM.
11. Ответ форматируется для Telegram HTML.
12. `/answer` может быть отправлен voice, если проходит локальную TTS-политику.
