...
- для `reply_to_bot` генерация ответа должна опираться в первую очередь на causal reply context: текущее сообщение, bot-сообщение, на которое отвечают, его parent-сообщение и связанный transcript;
