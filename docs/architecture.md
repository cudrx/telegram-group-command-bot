...
- запускает `/read`, `/meme` или генерацию ответа;
...
9. Для `/meme` запускается отдельный поток выбора через `meme-api.com`, скачивания media и отправки с локально отформатированной подписью.
10. Для `/summarize`, `/decide`, `/answer`, `/translate` собирается контекст, добавляется текущая дата и время Москвы, при необходимости добавляется контекст поиска и медиа, затем вызывается LLM.
