...
10. Для `/meme` запускается отдельный поток выбора через `meme-api.com`, скачивания media и отправки с локально отформатированной подписью.
...
- GIF/animation-мемы перед распознаванием превращаются в локальный JPEG-кадр примерно на первой секунде через `ffmpeg`, затем сохраняют image artifacts на сообщение бота;
...
- После успешной отправки image-мемы распознаются через Telegram `file_id`, а GIF/animation-мемы — через локальный JPEG-кадр; ошибка извлечения кадра не отменяет уже отправленный мем.
