...
- `/answer`, `/translate` и `/read` требуют reply на целевое сообщение.
...
- `/meme` делает внешний запрос к `meme-api.com`, скачивает image/gif media во временные файлы и должен чистить их после успешной отправки и после ошибок Telegram.
