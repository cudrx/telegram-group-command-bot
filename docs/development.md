...
- `/meme` делает внешний запрос к Reddit top-week listing, выбирает свежий supported image/video post, скачивает media во временные файлы и должен чистить их после успешной отправки и после ошибок Telegram. Для video-постов нужен `yt-dlp`, `ffmpeg` и `reddit-cookies.txt` рядом с SQLite базой.
