...
- Для Reddit video, Instagram Reels и YouTube Shorts не добавляйте прямое скачивание `fallback_url`/MP4 как shortcut или fallback: видео должно сразу идти через общий pipeline `yt-dlp download -> ffprobe inspect -> optional ffmpeg normalize -> sendVideo`, иначе легко отправить mp4 без audio track, с нестабильным aspect ratio или с pixel format, который Telegram показывает криво. Telegram-safe H.264/AAC MP4 с `yuv420p`, `SAR 1:1`, нормальным DAR и четными размерами не перекодируется. Unsafe-файлы нормализуются одним процессом за раз через `nice -n 10 ffmpeg -preset veryfast`, `color_range tv`, удаление metadata и `+faststart`.
