...
- Для Reddit video и Instagram Reels не добавляйте прямое скачивание `fallback_url`/MP4 как shortcut или fallback: видео должно сразу идти через `yt-dlp`, иначе легко отправить mp4 без audio track. Для Instagram Reels закрепленный selector policy — сначала предпочитать HLS/m3u8 video + m4a audio merge, как у Reddit video, чтобы мобильные Telegram-клиенты не ломали геометрию ролика. Для будущих YouTube Shorts используйте тот же принцип extractor-first.
