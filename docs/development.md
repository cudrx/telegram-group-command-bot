...
- Direct Reddit video link smoke: отправьте Reddit post URL с `reddit_video` в рабочий чат или личку администратора. Бот должен отправить `sendVideo` с title, subreddit и апвоутами, сохранить post metadata, очистить temp file и попытаться удалить исходное сообщение со ссылкой. В группе для удаления нужны admin-права бота и выключенный BotFather privacy mode, если ссылка отправляется без команды/упоминания.
