...
describe('buildIntentPrompt chat intent response shapes', () => {
  test('builds summarize prompt as chat-only compression without command arguments', () => {
    ...
    expect(prompt).toContain('<b>Кратко</b>');
    ...
    expect(prompt).toContain(
      'Insert one empty line between the final bullet and the final <b>Вывод</b> line.'
    );
    expect(prompt).toContain(
      'The final line must not repeat bullets or introduce new unrelated info.'
    );
    expect(prompt).toContain('No text before <b>Кратко</b>.');
    expect(prompt).toContain('No text after the final <b>Вывод</b> line.');
    expect(prompt).toContain(
    ...
    expect(prompt).not.toContain('optional meaningful <b>Вывод</b>');
  ...
  test('builds decide prompt for chat disputes without external knowledge', () => {
    ...
    expect(prompt.indexOf('<b>Позиции</b>')).toBeLessThan(
      prompt.indexOf('<b>Аргументы</b>')
    );
    expect(prompt.indexOf('<b>Аргументы</b>')).toBeLessThan(
