...
describe("generate deploy metadata", () => {
  test("creates metadata from a push commit range", () => {
    const gitLog = vi.fn().mockReturnValue("fix: captions\nfeat: announcements\n");
    ...
    expect(gitLog).toHaveBeenCalledWith("before-sha..f3f896a0c03d0c109db633a6182e798e8ca0b96f");
  ...
  test("uses the current commit when before sha is all zeroes", () => {
    ...
    expect(gitLog).toHaveBeenCalledWith("f3f896a0c03d0c109db633a6182e798e8ca0b96f^..f3f896a0c03d0c109db633a6182e798e8ca0b96f");
  });
  ...
  test("writes metadata JSON to disk", () => {
    const directory = mkdtempSync(path.join(os.tmpdir(), "deploy-metadata-out-"));
